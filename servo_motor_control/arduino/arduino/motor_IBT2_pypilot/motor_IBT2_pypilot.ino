...
#define VNH2SP30 // defined for IBT-2 (BTS7960) board
#define DISABLE_TEMP_SENSE    // no temp sensors on this build
#define DISABLE_VOLTAGE_SENSE // no voltage sense hardware on this build
...
void setup()
{
    ...
    digitalWrite(clutch_pin, HIGH); // clutch disengaged at startup (active-low)
...
void disengage()
{
    ...
    TCCR2B = 0;
    clutch_start_time = 0;
    digitalWrite(clutch_pin, HIGH); // clutch disengaged (active-low) in disengage()
}
...
void engage()
{
    ...
    position(1000);
    digitalWrite(clutch_pin, LOW); // clutch engaged (active-low) in engage()
...
uint16_t TakeAmps(uint8_t p)
{
    ...
    // For IBT-2 / external shunt, do not use VNH2SP30 internal current sense.
    // Always use shunt-based scaling below, regardless of pwm_style.
...
void loop()
{
        ...
        if(++timeout_d >= 4/DIV_CLOCK) {
            ...
            if(clutch_start_time)
                if(--clutch_start_time == 0 && clutch_pwm < 250) {
                    OCR2A = clutch_pwm;
                    TCCR2A = _BV(WGM20) | _BV(COM2A1) | _BV(COM2A0); // phase correct pwm, inverted for active-low clutch
                }
    ...
    // test fault pins (NC switches: LOW = OK, HIGH = FAULT/open)
